...
for mtxfp in glob.glob("../cell_expression/*/*/*matrix.mtx.gz"):
    parts = mtxfp.split("/")
    mtxfn = parts[-1]
    oid = parts[-2]
    tp = parts[-3]
    assert tp.startswith("by_")
    tp = tp[3:]

    print(tp, oid, mtxfn)

    h5ad_fp = mtxfp.replace("_matrix.mtx.gz", ".h5ad")
    h5ad_fn = mtxfn.replace("_matrix.mtx.gz", ".h5ad")
    ...
    if not os.path.isfile(h5ad_fp):
        adata = load_by_sample_matrix(mtxfp) if tp == "sample" else load_by_species_matrix(mtxfp)
    ...
    config["meta"]["labels"][tp] = oid
    config["meta"]["labels"]["dataset"] = "zemke2023Conserved"
    config["meta"]["labels"]["technology"] = "10x genomics"
